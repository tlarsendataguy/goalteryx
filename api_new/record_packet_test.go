...
func TestRecordPacket(t *testing.T) {
	...
	recordPacket := api_new.NewRecordPacket(api_new.RecordCache(&data[0]), len(data), 1, false)
	rows := 0
...
func TestVarDataRecordPacket(t *testing.T) {
	...
	recordPacket := api_new.NewRecordPacket(api_new.RecordCache(&data[0]), len(data), 1, true)
	rows := 0
	for recordPacket.Next() {
