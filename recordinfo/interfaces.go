...
type FieldManager interface {
	// NumFields return the number of fields contained in the FieldManager.
	NumFields() int

	// HasVarFields returns true if the FieldManager contains variable-length fields
	HasVarFields() bool
	...
	// GetFieldByName returns field information of the field with the specified name.  If FieldManager does not
	...
	// GetFieldTypeByName returns the field type of the field with the specified name.  If FieldManager does not
	// contains a field with the specified name, an error is returned with an empty FieldInfo struct.
	GetFieldTypeByName(name string) (string, error)

	// FixedSize returns the total length of the fixed-size portion of the FieldManager object.  This is used to
	// identify the fixed portion of the generated record blobs.
	FixedSize() int
}

// RecordBlobReader is the interface which defines all of the behaviors needed to read Alteryx record blobs.
type RecordBlobReader interface {
	FieldManager
...
// RecordInfo is the interface which defines all of the behaviors needed to generate Alteryx records.
type RecordInfo interface {
	...
	GetCurrentNull(fieldName string) (bool, error)
	...
	// GenerateRecordBlobReader outputs a RecordBlobReader that can read records generated by the RecordInfo object.
	GenerateRecordBlobReader() RecordBlobReader
}
...
type Generator interface {
	...
	// GenerateRecordBlobReader generates a RecordBlobReader object from the current fieldset.
	GenerateRecordBlobReader() RecordBlobReader
}
